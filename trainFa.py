...
def train(train_batches):
    ...
    for k in shuffle_index[:30]:
        ...
        invtau_and_distribution = torch.zeros((12, num_peds, 3))
        ...
        prediction, w_v, invtau_and_distribution[0] = model.forward_next_step(current_step, current_vel, initial_speeds, dest,
        ...
        for t in range(params['future_length'] - 1):
            ...
            prediction, w_v, invtau_and_distribution[t+1] = model.forward_next_step(current_step, current_vel, initial_speeds, dest,
                                                      outputs_features, device=device)
            ...
            current_vel = w_v # peds*2
        optimizer.zero_grad()
        # prior = torch.tensor([[0.9029, 0.9029, 0.9029, 0.9029, 0.9029, 0.9029, 0.9029, 0.9029, 0.9028, 0.9028, 0.9028, 0.9028],
        #                       [0.1043, 0.1044, 0.1044, 0.1044, 0.1044, 0.1044, 0.1044, 0.1044, 0.1044, 0.1043, 0.1043, 0.1043]])
        prior = torch.tensor([[1.1216, 1.1216, 1.1216, 1.1216, 1.1216, 1.1216, 1.1217, 1.1217, 1.1217, 1.1217, 1.1218, 1.1218],
                              [0.1255, 0.1256, 0.1256, 0.1256, 0.1256, 0.1256, 0.1256, 0.1256, 0.1255, 0.1255, 0.1255, 0.1254]])

        loss = calculate_loss(criterion, future, predictions, invtau_and_distribution, prior)
        loss.backward()
...
def test(traj, generated_goals):
    ...
    with torch.no_grad():
        ...
        predictions = torch.zeros(num_peds, params['future_length'], 2).to(device)
        miu_tau_all = torch.zeros((num_peds, 12))
        sigma_tau_all = torch.zeros((num_peds, 12))

        prediction, w_v = model.forward_next_step(current_step, current_vel, initial_speeds, dest,
                                              outputs_features,infer=True, device=device)
...
parser.add_argument('--save_file', '-sf', type=str, default='SDD_goals_1.pt')
...
model = BNSP_attraction(params["input_size"], params["embedding_size"], params["rnn_size"], params["output_size"],  params["enc_dest_state_size"], params["dec_tau_size"])
model = model.double().to(device)
load_path = 'saved_models/SDD_goals_6.52_invtau.pt'
checkpoint_trained = torch.load(load_path, map_location=torch.device('cpu'))
